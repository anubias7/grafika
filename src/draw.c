...
double sizeOfRoom = 440;

GLfloat material_ambient_default[] = {0.9, 0.9, 0.9, 0.5};
GLfloat material_diffuse_default[] = {1, 0, 0, 1};
GLfloat material_specular_default[] = {1, 1, 1, 1};
GLfloat material_ambient_golf[] = {1, 0, 0, 1};
...
void draw_content(World* world)
{
	...
	glPushMatrix();
		...
		glMaterialfv(GL_FRONT, GL_AMBIENT, world->beachball.material_ambient);
		glBindTexture(GL_TEXTURE_2D, world->beachball.texture);
		glRotatef(270.0, 1.0, 0.0, 0.0);
		glRotatef(90.0, 0.0, 0.0, 1.0);
		glScalef(0.3f, 0.3f, 0.3f);

		draw_model(&world->beachball.model);
    glPopMatrix();
	
	glPushMatrix();
		glTranslatef(world->soccerball.position.x, world->soccerball.position.y, world->soccerball.position.z);

		glMaterialfv(GL_FRONT, GL_AMBIENT, world->soccerball.material_ambient);
		
		glBindTexture(GL_TEXTURE_2D, world->soccerball.texture);
		glRotatef(270.0, 1.0, 0.0, 0.0);
		glRotatef(90.0, 0.0, 0.0, 1.0);
		glScalef(0.7f, 0.7f, 0.7f);

		draw_model(&world->soccerball.model);
    glPopMatrix();
	
	glPushMatrix();
		glTranslatef(world->golf.position.x, world->golf.position.y, world->golf.position.z);

		glMaterialfv(GL_FRONT, GL_AMBIENT, material_ambient_golf);
		glMaterialfv(GL_FRONT, GL_DIFFUSE, material_diffuse_default);
		glMaterialfv(GL_FRONT, GL_SPECULAR, material_specular_default);
		
		glBindTexture(GL_TEXTURE_2D, world->golf.texture);
		glRotatef(270.0, 1.0, 0.0, 0.0);
		glRotatef(90.0, 0.0, 0.0, 1.0);
		glScalef(2.5f, 2.5f, 2.5f);

		draw_model(&world->golf.model);
    glPopMatrix();
